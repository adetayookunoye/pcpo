...
While isolated existence proofs for potential-based and projection-based approximations exist \citep{richter2022neural, xu2025physically}, we provide a unified theoretical framework (\textbf{Theorems 1 \& 2}) that establishes the equivalence of these approaches for Neural Operators. Crucially, we extend this theory to the probabilistic setting, proving that our stream-function-based cVAE (Pattern A) retains the universal approximation property over the distribution of divergence-free fields, a result previously unestablished for probabilistic operator learning.

Our contributions beyond this theoretical foundation include: (i) a clean, production-ready implementation of stream-function FNO for 2D incompressible flows on PDEBench, (ii) the first probabilistic operator (cVAE-FNO) maintaining hard constraints via stream-function parameterization, (iii) rigorous multi-seed empirical comparison with bootstrap confidence intervals, and (iv) spatially adaptive constraint weighting for handling heterogeneous constraint violations across the domain.
